...
def _preset_iteration_candidates(iteration: int) -> tuple[ExperimentCandidate, ...] | None:
    ...
    if 106 <= iteration <= 115:
        return _reentry_tuning_candidates(iteration)
    if 116 <= iteration <= 125:
        return _broad_risk_on_reentry_candidates(iteration)
    if 126 <= iteration <= 135:
...
def _reentry_tuning_candidates(iteration: int) -> tuple[ExperimentCandidate, ...]:
    """Focused tests for getting risk back on earlier after large drawdowns.

    These iterations start from the high-CAGR AI escape engine and test one
    explicit failure mode: the engine can get defensive quickly, but it can miss
    the first part of the recovery because trend and scenario gates wait for too
    much confirmation.
    """

    base = _operating_system_candidates()[0]
    assert base.name == "i21_os_ai_escape_scenario_sized"
    ai_core = ["QQQ", "SMH", "SOXX", "IGV", "NVDA", "AVGO", "MSFT", "META", "AMZN", "PLTR"]
    ai_concentrated = ["QQQ", "SMH", "SOXX", "NVDA", "AVGO", "MSFT", "META", "AMZN"]
    broad_recovery = ["SPY", "QQQ", "RSP", "IWM", "XLK", "MTUM", "QUAL", "COWZ", "HYG", "LQD"]
    sector_recovery = ["XLK", "SMH", "SOXX", "IGV", "XLY", "XLC", "XLI", "XLF", "RSP", "HYG", "LQD"]
    ...
    def direct(
        *,
        ...
        family: str,
        hypothesis: str,
        scenario_sizing: ScenarioSizingConfig | None = base.scenario_sizing,
        decision_sanity: DecisionSanityConfig | None = None,
        future_state_model: FutureStateModelConfig | None = None,
        strategy_drawdown_model: StrategyDrawdownModelConfig | None = None,
        **updates: object,
        ...
        return _candidate(
            name=name,
            role="reentry_tuning_candidate",
            phase="reentry_tuning",
            family=family,
            parent="i84_high_cagr_control_raw_ai_escape",
            hypothesis=hypothesis,
            scenario_sizing=scenario_sizing,
            decision_sanity=decision_sanity,
            future_state_model=future_state_model,
    ...
    batches: dict[int, tuple[ExperimentCandidate, ...]] = {
        106: (
            direct(
                name="i106_reentry_fast_42d_low_hurdle",
                family="reentry_fast_signal",
                hypothesis="Shorten the AI escape lookback and lower the return hurdle so recovery can qualify sooner.",
                lookback_days=42,
                skip_days=0,
                min_return=0.010,
                trend_filter_days=63,
            ...
            direct(
                ...
                family="reentry_fast_signal",
                hypothesis="A very fast 21-day pulse tests whether early repair should get a starter risk budget.",
                lookback_days=21,
                skip_days=0,
                min_return=0.000,
                trend_filter_days=42,
            ...
            direct(
                ...
                family="reentry_fast_signal",
                hypothesis="Keep the 63-day parent window but remove the skip period to reduce re-entry delay.",
                skip_days=0,
                min_return=0.000,
                trend_filter_days=63,
                volatility_target=VolatilityTargetConfig(annualized_volatility=0.16, lookback_days=42),
            ...
            direct(
                ...
                family="reentry_fast_signal",
                hypothesis="Use a wider equal-weight winner basket so stale single-name signals do not block recovery capture.",
                lookback_days=42,
                skip_days=0,
                min_return=0.005,
                top_n=6,
            ...
            direct(
                name="i106_reentry_fast_no_scenario_control",
                family="reentry_fast_signal",
                hypothesis="Remove scenario sizing to test whether delayed re-entry is mostly scenario drag.",
                scenario_sizing=None,
                lookback_days=42,
                skip_days=0,
                min_return=0.005,
        ...
        107: (
            _dip_overlay_candidate(
                name="i107_reentry_panic_starter_ai",
                family="reentry_panic_starter",
                hypothesis="Replace cash with an AI starter sleeve after moderate discounts, then step up if repair confirms.",
                tickers=ai_core,
                lookback_days=42,
                skip_days=0,
                top_n=4,
            ...
            _dip_overlay_candidate(
                name="i107_reentry_broad_index_bridge",
                family="reentry_broad_bridge",
                hypothesis="Use SPY/QQQ/RSP/IWM as the first recovery bridge before single-name AI fully requalifies.",
                tickers=broad_recovery,
                lookback_days=42,
                skip_days=0,
                top_n=5,
                min_return=0.000,
            ...
            _dip_overlay_candidate(
                name="i107_reentry_relaxed_confirmation",
                family="reentry_panic_starter",
                hypothesis="Relax breadth confirmation to catch early V-shaped recoveries while still requiring credit repair.",
                tickers=ai_core,
                lookback_days=42,
                skip_days=0,
                top_n=4,
        ...
        108: (
            _dip_overlay_candidate(
                name="i108_reentry_strict_credit_breadth_gate",
                family="reentry_false_bottom_control",
                hypothesis="Strict credit and breadth gates test whether fast re-entry is only safe after broad repair.",
                tickers=broad_recovery,
                lookback_days=63,
                skip_days=5,
                top_n=5,
            ...
            _dip_overlay_candidate(
                name="i108_reentry_two_week_repair_gate",
                family="reentry_false_bottom_control",
                hypothesis="Require a two-week repair window before redeploying materially into the rebound.",
                tickers=ai_core,
                lookback_days=63,
                skip_days=5,
                top_n=4,
            ...
            _dip_overlay_candidate(
                name="i108_reentry_vol_compression_gate",
                family="reentry_false_bottom_control",
                hypothesis="Only buy early dips when volatility is already compressing enough to avoid falling knives.",
                tickers=ai_core,
                lookback_days=42,
                skip_days=0,
                top_n=4,
            ...
            _dip_overlay_candidate(
                name="i108_reentry_deep_only_rare_event",
                family="reentry_false_bottom_control",
                hypothesis="Buy near-bottom only after deep discounts; avoid every ordinary pullback.",
                tickers=ai_core,
                lookback_days=84,
                skip_days=5,
                top_n=4,
        ...
        109: (
            _ai_cycle_candidate(
                name="i109_reentry_asymmetric_ai_fast",
                family="reentry_asymmetric_cycle",
                hypothesis="Let AI re-enter faster from cash than it is added during normal risk-on periods.",
                core_tickers=["SPY", "RSP", "IWM", "QUAL", "USMV", "GLD", "TLT", "HYG", "LQD"],
                satellite_tickers=ai_core,
                lookback_days=42,
                skip_days=0,
            ...
            _ai_cycle_candidate(
                name="i109_reentry_asymmetric_broad_first",
                family="reentry_asymmetric_cycle",
                hypothesis="Use a broad equity core for the first recovery leg, then let AI satellite scale.",
                core_tickers=broad_recovery,
                satellite_tickers=ai_concentrated,
                lookback_days=42,
                skip_days=0,
                top_n=5,
            ...
            _ai_cycle_candidate(
                name="i109_reentry_ai_credit_unlock",
                family="reentry_asymmetric_cycle",
                hypothesis="Allow large AI re-entry only when credit repair is visible, even if breadth lags.",
                core_tickers=["HYG", "JNK", "LQD", "BKLN", "SRLN", "SPY", "RSP", "GLD", "IEF"],
                satellite_tickers=ai_core,
                lookback_days=42,
                skip_days=0,
                top_n=4,
            ...
            _ai_cycle_candidate(
                name="i109_reentry_ai_slow_core_fast_satellite",
                family="reentry_asymmetric_cycle",
                hypothesis="Keep the defensive core slow, but let the AI satellite use fast repair evidence.",
                core_tickers=["SPY", "RSP", "IWM", "GLD", "TLT", "IEF", "HYG", "LQD"],
                satellite_tickers=ai_core,
                lookback_days=84,
                skip_days=10,
                top_n=4,
        ...
        110: (
            direct(
                name="i110_reentry_2011_broadening_equal_weight",
                family="reentry_2011_repair",
                hypothesis="Reduce the 2011-style narrow-stock problem with a broader equal-weight recovery basket.",
                tickers=[*ai_core, "SPY", "RSP", "XLK", "MTUM", "QUAL", "COWZ"],
                lookback_days=42,
                skip_days=0,
                top_n=7,
                min_return=0.000,
            ...
            direct(
                ...
                family="reentry_2011_repair",
                hypothesis="Use a broad QQQ/XLK/MTUM proxy set so single-name weakness cannot dominate recovery timing.",
                tickers=["QQQ", "XLK", "MTUM", "SPY", "RSP", "SMH", "SOXX", "IGV", "HYG", "LQD"],
                lookback_days=42,
                skip_days=0,
                top_n=5,
                min_return=0.000,
            ...
            direct(
                name="i110_reentry_2011_quality_bridge",
                family="reentry_2011_repair",
                hypothesis="Bridge early recovery through quality/value/cash-flow ETFs before concentrated AI leaders reassert.",
                tickers=["QQQ", "SPY", "RSP", "QUAL", "USMV", "COWZ", "MTUM", "XLK", "HYG", "LQD"],
                lookback_days=42,
                skip_days=0,
                top_n=6,
                min_return=0.000,
            ...
            _dip_overlay_candidate(
                name="i110_reentry_2011_broad_bridge_overlay",
                family="reentry_2011_repair",
                hypothesis="Overlay the broad recovery bridge onto the AI engine to reduce long defensive stalls.",
                tickers=list(dict.fromkeys([*broad_recovery, *ai_core])),
                lookback_days=42,
                skip_days=0,
                top_n=6,
                min_return=0.000,
        ...
        111: (
            ...
            direct(
                ...
                family="reentry_vol_drag",
                hypothesis="Remove the volatility target to measure pure re-entry drag versus risk-control benefit.",
                volatility_target=None,
            ),
            direct(
                name="i111_reentry_vol_target_18_fast_signal",
                family="reentry_vol_drag",
                hypothesis="Combine faster trend qualification with a higher vol target for maximum recovery capture.",
                lookback_days=42,
                skip_days=0,
                min_return=0.005,
                trend_filter_days=63,
        ...
        112: (
            direct(
                name="i112_reentry_ml_tail_gate_loose",
                family="reentry_ml_less_sticky",
            ...
            direct(
                name="i112_reentry_ml_extra_trees_loose",
                family="reentry_ml_less_sticky",
                hypothesis="Extra-trees loose tail gate tests nonlinear risk warnings without delaying re-entry.",
                decision_sanity=_decision_sanity_profile("wide_cap"),
                future_state_model=_return_preserving_ml_profile(
                    "sk_extra_trees", horizon_days=21, feature_set="ai", threshold=0.52, stress=0.78
                ),
            ),
            direct(
                name="i112_reentry_ml_short_horizon_tail",
                family="reentry_ml_less_sticky",
            ...
            direct(
                name="i112_reentry_strategy_dd_guard_high_floor",
                family="reentry_ml_less_sticky",
                hypothesis="A strategy-specific drawdown guard with a high floor should reduce tails without killing re-entry.",
                strategy_drawdown_model=_strategy_drawdown_profile(
                    "sk_random_forest",
                    horizon_days=21,
                    feature_set="ai",
                    threshold=-0.12,
        ...
        113: (
            _sector_regime_candidate(
                name="i113_reentry_sector_growth_repair",
                family="reentry_sector_vehicle",
                hypothesis="Find the best recovery vehicle across growth, semis, software, communication, and broad indices.",
                tickers=sector_recovery,
                lookback_days=42,
                skip_days=0,
                top_n=5,
                min_return=0.000,
        ...
        114: (
            _ai_cycle_candidate(
                name="i114_reentry_operable_fast_weekly",
                family="reentry_operability",
                hypothesis="Fast re-entry with moderate step caps tests whether it can be operated manually.",
                core_tickers=broad_recovery,
                satellite_tickers=ai_core,
                lookback_days=42,
                skip_days=0,
            ...
            _ai_cycle_candidate(
                name="i114_reentry_operable_two_step",
                family="reentry_operability",
                hypothesis="Two-step re-entry caps each move while still starting earlier after the trough.",
                core_tickers=broad_recovery,
                satellite_tickers=ai_core,
                lookback_days=42,
                skip_days=0,
            ...
            direct(
                name="i114_reentry_operable_fast_hysteresis",
                family="reentry_operability",
                hypothesis="Direct AI escape with fast signals but larger action thresholds to reduce daily twitch.",
                lookback_days=42,
                skip_days=0,
                min_return=0.005,
                trend_filter_days=63,
            ...
            _dip_overlay_candidate(
                ...
                family="reentry_operability",
                hypothesis="Overlay re-entry with weekly-ish confirmation and capped steps for human execution.",
                tickers=list(dict.fromkeys([*broad_recovery, *ai_core])),
                lookback_days=63,
                skip_days=5,
                top_n=6,
        ...
        115: (
            direct(
                name="i115_reentry_final_fast_broad_ai",
                family="reentry_final_composite",
                hypothesis="Composite: fast AI escape, broad recovery bridge, higher vol budget, and balanced scenario sizing.",
                tickers=[*ai_core, "SPY", "RSP", "XLK", "MTUM", "QUAL", "COWZ"],
                lookback_days=42,
                skip_days=0,
                top_n=6,
            ...
            _dip_overlay_candidate(
                ...
                family="reentry_final_composite",
                hypothesis="Composite overlay: buy broad bridge first, then let AI/semis win if they confirm.",
                tickers=list(dict.fromkeys([*broad_recovery, *ai_core])),
                lookback_days=42,
                skip_days=0,
                top_n=6,
            ...
            _ai_cycle_candidate(
                name="i115_reentry_final_asymmetric_cycle",
                family="reentry_final_composite",
                hypothesis="Composite cycle: asymmetric AI satellite re-entry with broad core and manual step caps.",
                core_tickers=broad_recovery,
                satellite_tickers=ai_core,
                lookback_days=42,
                skip_days=0,
            ...
            _sector_regime_candidate(
                name="i115_reentry_final_sector_vehicle",
                family="reentry_final_composite",
                hypothesis="Composite sector vehicle: let the recovery choose between AI, growth, broadening, and defense.",
                tickers=[*sector_recovery, "VRT", "ETN", "PWR", "CEG", "QUAL", "COWZ"],
                lookback_days=42,
                skip_days=0,
                top_n=6,
                min_return=0.000,
...
def _broad_risk_on_reentry_candidates(iteration: int) -> tuple[ExperimentCandidate, ...]:
    """Re-entry tests that deliberately reduce narrow AI-beta dependence.

    The prior re-entry block found promising behavior in AI-escape variants:
    they often de-risk early enough, but can re-enter too late and sometimes
    depend on a narrow technology/AI recovery sleeve. These iterations ask
    whether the same risk-off/risk-on logic works when the risk-on side is
    broad indexes, equal-weight equities, factors, sectors, cyclicals, or global
    equities instead of concentrated AI beta.
    """
    ...
    def broad_dual(
        *,
        ...
        max_asset_weight: float | None = 0.30,
        trend_filter_days: int | None = 100,
        vol_target: float | None = 0.16,
        ...
        return _candidate(
            name=name,
            role="broad_reentry_candidate",
            phase="broad_risk_on_reentry",
            family=family,
            parent="i84_high_cagr_control_raw_ai_escape",
            hypothesis=hypothesis,
            scenario_sizing=scenario_sizing,
    ...
    batches: dict[int, tuple[ExperimentCandidate, ...]] = {
        116: (
            broad_dual(
                name="i116_broad_index_fast_reentry",
                family="broad_index_reentry",
                hypothesis="Use diversified U.S. equity indexes as the first recovery sleeve instead of concentrated AI beta.",
                tickers=index_core,
                lookback_days=42,
                skip_days=0,
                top_n=4,
            ...
            broad_dual(
                name="i116_broad_index_no_qqq_reentry",
                family="broad_index_reentry_no_ai",
                hypothesis="Remove QQQ entirely to test whether broad re-entry works without the Nasdaq/AI tailwind.",
                tickers=index_no_ai,
                lookback_days=42,
                skip_days=0,
                top_n=4,
            ...
            _dip_overlay_candidate(
                name="i116_broad_index_dip_bridge",
                family="broad_index_reentry",
                hypothesis="Buy broad equity dips in metered steps before narrower growth leadership has fully confirmed.",
                tickers=unique([*index_core, "HYG", "LQD"]),
                lookback_days=42,
                skip_days=0,
                top_n=5,
                min_return=0.000,
            ...
            broad_dual(
                name="i116_equal_weight_broadening_fast",
                family="broad_index_reentry",
                hypothesis="Favor equal-weight broadening so a recovery can be owned before mega-cap tech retakes leadership.",
                tickers=["RSP", "IWM", "MDY", "SPY", "VTI", "XLF", "XLI", "XLY"],
                lookback_days=42,
                skip_days=0,
                top_n=5,
                min_return=0.000,
        ...
        117: (
            broad_dual(
                name="i117_factor_momentum_reentry",
                family="factor_reentry",
                hypothesis="Let factor ETFs express risk-on recovery so the sleeve can pivot among growth, value, quality, and dividends.",
                tickers=factors,
                lookback_days=42,
                skip_days=0,
                top_n=5,
            ...
            broad_dual(
                name="i117_quality_value_reentry_no_ai",
                family="factor_reentry_no_ai",
                hypothesis="Test a no-QQQ quality/value/cash-flow recovery basket for AI-bubble drawdown resilience.",
                tickers=quality_value,
                lookback_days=42,
                skip_days=0,
                top_n=5,
                min_return=0.000,
            ...
            _dip_overlay_candidate(
                name="i117_factor_dip_repair",
                family="factor_reentry",
                hypothesis="Use factor repair evidence after broad drawdowns, with quality/value allowed to lead early re-risking.",
                tickers=unique([*factors, "SPY", "RSP", "HYG", "LQD"]),
                lookback_days=42,
                skip_days=0,
                top_n=6,
                min_return=0.000,
        ...
        118: (
            _sector_regime_candidate(
                name="i118_sector_rotation_reentry",
                family="sector_reentry",
                hypothesis="Let sector leadership choose the recovery vehicle rather than assuming the recovery is AI-led.",
                tickers=sectors,
                lookback_days=42,
                skip_days=0,
                top_n=5,
            ...
            _sector_regime_candidate(
                name="i118_sector_cyclicals_reentry",
                family="cyclical_reentry",
                hypothesis="Test whether cyclical sectors and banks are better early recovery tells than AI leadership.",
                tickers=cyclicals,
                lookback_days=42,
                skip_days=0,
                top_n=5,
            ...
            broad_dual(
                name="i118_sector_equal_weight_reentry",
                family="sector_reentry",
                hypothesis="A fast equal-weight sector basket checks whether sector diversification fixes the 2011-style drag.",
                tickers=sectors,
                lookback_days=42,
                skip_days=0,
                top_n=6,
        ...
        119: (
            broad_dual(
                name="i119_cyclical_broadening_fast",
                family="cyclical_reentry",
                hypothesis="Buy recoveries through cyclicals, small/mid caps, housing, retail, transports, and banks.",
                tickers=cyclicals,
                lookback_days=42,
                skip_days=0,
                top_n=5,
            ...
            _dip_overlay_candidate(
                name="i119_cyclical_dip_washout",
                family="cyclical_reentry",
                hypothesis="Test whether deep cyclical washouts should be bought in steps when credit stops worsening.",
                tickers=unique([*cyclicals, "HYG", "LQD"]),
                lookback_days=42,
                skip_days=0,
                top_n=5,
                min_return=0.000,
            ...
            _sector_regime_candidate(
                name="i119_small_mid_bank_reentry",
                family="cyclical_reentry",
                hypothesis="Allow small/mid caps and banks to lead if the market broadens after a drawdown.",
                tickers=["IWM", "MDY", "RSP", "XLF", "KRE", "XLI", "XHB", "XRT", "IYT"],
                lookback_days=42,
                skip_days=0,
                top_n=4,
                min_return=0.000,
            ...
            broad_dual(
                name="i119_cyclical_low_churn",
                family="cyclical_reentry_operable",
                hypothesis="Add hysteresis so cyclical broadening is tradable by a human and not a daily whipsaw machine.",
                tickers=cyclicals,
                lookback_days=63,
                skip_days=0,
                top_n=5,
        ...
        120: (
            broad_dual(
                name="i120_global_equity_reentry",
                family="global_reentry",
                hypothesis="Use global equity leadership to avoid assuming every recovery must come from U.S. mega-cap technology.",
                tickers=global_equity,
                lookback_days=42,
                skip_days=0,
                top_n=5,
                min_return=0.000,
            ...
            _dip_overlay_candidate(
                name="i120_global_dip_repair",
                family="global_reentry",
                hypothesis="After U.S. drawdowns, let global markets participate if they show better repair than AI-heavy U.S. growth.",
                tickers=unique([*global_equity, "HYG", "LQD"]),
                lookback_days=42,
                skip_days=0,
                top_n=6,
                min_return=0.000,
            ...
            _sector_regime_candidate(
                name="i120_global_us_bridge",
                family="global_reentry",
                hypothesis="Regime-rotate between U.S. broadening and global risk-on when leadership shifts geographically.",
                tickers=unique([*global_equity, "VTV", "COWZ", "QUAL"]),
                lookback_days=42,
                skip_days=0,
                top_n=5,
                min_return=0.000,
        ...
        121: (
            broad_dual(
                name="i121_anti_ai_value_quality_reentry",
                family="anti_ai_reentry",
                hypothesis="Explicitly avoid QQQ/semis/single-name AI and test whether broad value-quality recovery still works.",
                tickers=anti_ai,
                lookback_days=42,
                skip_days=0,
                top_n=5,
            ...
            _dip_overlay_candidate(
                name="i121_anti_ai_dip_reentry",
                family="anti_ai_reentry",
                hypothesis="Use metered dip buying in a no-AI basket as a stress test against AI-bubble unwinds.",
                tickers=unique([*anti_ai, "HYG", "LQD"]),
                lookback_days=42,
                skip_days=0,
                top_n=5,
                min_return=0.000,
        ...
        122: (
            broad_dual(
                name="i122_quality_value_fast_bridge",
                family="quality_value_bridge",
                hypothesis="Quality/value/cash-flow ETFs may be the safer bridge before higher-beta equities are reintroduced.",
                tickers=unique([*quality_value, "SPY", "RSP", "IWM"]),
                lookback_days=42,
                skip_days=0,
                top_n=6,
                min_return=0.000,
            ...
            broad_dual(
                name="i122_quality_value_no_vol_target",
                family="quality_value_bridge",
                hypothesis="A no-vol-target ablation tests whether upside is being capped too aggressively during recoveries.",
                tickers=unique([*quality_value, "SPY", "RSP", "IWM"]),
                lookback_days=42,
                skip_days=0,
                top_n=6,
            ...
            _dip_overlay_candidate(
                name="i122_quality_value_dip_bridge",
                family="quality_value_bridge",
                hypothesis="Metered dip re-entry through quality/value asks whether we can buy weakness without catching a falling knife.",
                tickers=unique([*quality_value, "SPY", "RSP", "HYG", "LQD"]),
                lookback_days=42,
                skip_days=0,
                top_n=6,
                min_return=0.000,
                ...
                step=0.24,
                max_risk=0.92,
                max_asset_weight=0.21,
                vol_ceiling=0.40,
                scenario_sizing=_scenario_profile("balanced"),
            ),
            broad_dual(
                name="i122_quality_value_low_churn",
                family="quality_value_bridge_operable",
                hypothesis="Low-churn quality/value bridge tests whether good re-entry behavior can be human-tradable.",
                tickers=unique([*quality_value, "SPY", "RSP", "IWM"]),
                lookback_days=63,
                skip_days=0,
                top_n=6,
        ...
        123: (
            broad_dual(
                name="i123_diversified_high_beta_capped",
                family="diversified_high_beta_reentry",
                hypothesis="If upside capture is the problem, test high-beta recovery without relying on a narrow AI basket.",
                tickers=diversified_high_beta,
                lookback_days=42,
                skip_days=0,
                top_n=4,
                min_return=0.005,
            ...
            _dip_overlay_candidate(
                name="i123_high_beta_dip_metered",
                family="diversified_high_beta_reentry",
                hypothesis="Buy diversified high-beta recoveries in steps only after a discount and initial repair.",
                tickers=unique([*diversified_high_beta, "HYG", "LQD"]),
                lookback_days=42,
                skip_days=0,
                top_n=4,
                min_return=0.005,
                ...
                step=0.24,
                max_risk=0.90,
                max_asset_weight=0.19,
                vol_ceiling=0.50,
                scenario_sizing=_scenario_profile("balanced"),
            ),
            broad_dual(
                name="i123_high_beta_no_scenario_ablation",
                family="diversified_high_beta_reentry",
                hypothesis="Remove scenario drag to test whether high-beta broad recovery can keep CAGR without permanent bearishness.",
                tickers=diversified_high_beta,
                lookback_days=42,
                skip_days=0,
                top_n=4,
            ...
            broad_dual(
                name="i123_high_beta_low_churn_capped",
                family="diversified_high_beta_reentry_operable",
                hypothesis="A capped and smoothed high-beta sleeve tests upside capture with fewer tiny human trades.",
                tickers=diversified_high_beta,
                lookback_days=42,
                skip_days=0,
                top_n=4,
        ...
        124: (
            broad_dual(
                name="i124_operable_broad_index_low_churn",
                family="broad_reentry_operable",
                hypothesis="Convert broad index re-entry into a low-churn paper-monitorable system.",
                tickers=index_core,
                lookback_days=63,
                skip_days=0,
                top_n=5,
            ...
            broad_dual(
                name="i124_operable_factor_low_churn",
                family="factor_reentry_operable",
                hypothesis="Convert factor re-entry into a low-churn candidate with explicit decision sanity checks.",
                tickers=factors,
                lookback_days=63,
                skip_days=0,
                top_n=6,
            ...
            _sector_regime_candidate(
                name="i124_operable_sector_low_churn",
                family="sector_reentry_operable",
                hypothesis="Convert sector re-entry into a human-tradable rotation with minimum move and hold constraints.",
                tickers=sectors,
                lookback_days=63,
                skip_days=0,
                top_n=5,
            ...
            broad_dual(
                name="i124_operable_anti_ai_low_churn",
                family="anti_ai_reentry_operable",
                hypothesis="A no-AI low-churn operating candidate checks whether broad recovery can be monitored without AI-bubble exposure.",
                tickers=anti_ai,
                lookback_days=63,
                skip_days=0,
                top_n=6,
        ...
        125: (
            _ai_cycle_candidate(
                name="i125_broad_factor_satellite_cycle",
                family="broad_reentry_final_composite",
                hypothesis="Use the cycle overlay with broad/factor satellites rather than AI satellites to test generalized re-risking.",
                core_tickers=["SPY", "RSP", "IWM", "MDY", "HYG", "LQD", "GLD", "TLT"],
                satellite_tickers=["VUG", "MTUM", "QUAL", "COWZ", "VTV", "XLF", "XLI", "XLY"],
                lookback_days=42,
                skip_days=0,
                top_n=5,
                min_return=0.000,
                satellite_max=0.62,
                satellite_risk_on=0.36,
                satellite_reentry=0.78,
                trigger=-0.08,
                deep=-0.22,
                min_recovery=0.006,
            ...
            _ai_cycle_candidate(
                name="i125_anti_ai_satellite_cycle",
            ...
            broad_dual(
                name="i125_broad_reentry_final_dual",
                family="broad_reentry_final_composite",
                hypothesis="Final broad dual candidate: fast enough to re-enter, diversified enough to avoid AI concentration, and capped enough for operability.",
                tickers=unique([*index_core, *factors, *cyclicals]),
                lookback_days=42,
                skip_days=0,
                top_n=8,
                min_return=0.000,
            ...
            _dip_overlay_candidate(
                name="i125_broad_reentry_final_dip_overlay",
                family="broad_reentry_final_composite",
                hypothesis="Final broad dip overlay: buy broad/factor/cyclical recovery in steps after discount and repair confirmation.",
                tickers=unique([*index_core, *factors, *cyclicals, "HYG", "LQD"]),
                lookback_days=42,
                skip_days=0,
                top_n=8,
                min_return=0.000,
...
def _high_cagr_broadened_reentry_candidates(iteration: int) -> tuple[ExperimentCandidate, ...]:
    """Higher-upside re-entry tests with broader equity expression.

    The broad-only pass proved that reducing AI concentration can preserve
    drawdown control, but it cut CAGR too far. This block keeps the high-CAGR
    objective explicit while testing whether upside can come from broad growth,
    high-beta ETFs, quality compounders, sector/thematic recovery, and capped AI
    ...
    def high_cagr_dual(
        *,
        ...
        min_return: float = 0.0,
        weighting: str = "risk_adjusted_score",
        max_asset_weight: float | None = 0.24,
        ...
        return _candidate(
            name=name,
            role="high_cagr_broadened_reentry",
            phase="high_cagr_broadened_reentry",
            family=family,
            parent="i84_high_cagr_control_raw_ai_escape",
            hypothesis=hypothesis,
            scenario_sizing=scenario_sizing,
    ...
    batches: dict[int, tuple[ExperimentCandidate, ...]] = {
        126: (
            high_cagr_dual(
                name="i126_broad_growth_high_vol_target",
                family="broad_growth_high_cagr",
                hypothesis="Keep upside by using broad growth and momentum ETFs with higher vol budget and capped single-ETF weights.",
            ...
            high_cagr_dual(
                name="i126_broad_growth_sanity_sized",
                family="broad_growth_high_cagr",
                hypothesis="Use broad growth but only let scenario pressure de-risk modestly through decision sanity.",
                tickers=broad_growth,
                top_n=5,
                max_asset_weight=0.24,
                vol_target=0.20,
            ...
            high_cagr_dual(
                name="i126_broad_growth_no_single_ai",
                family="broad_growth_reduced_single_name_ai",
                hypothesis="Use broad growth and thematic ETFs but avoid individual AI winners as the return engine.",
                tickers=broad_growth_no_single_ai,
                top_n=5,
                max_asset_weight=0.22,
                vol_target=0.22,
                scenario_sizing=None,
            ...
            _dip_overlay_candidate(
                name="i126_broad_growth_dip_reentry",
                family="broad_growth_high_cagr",
                hypothesis="Buy broad growth drawdowns in steps while retaining enough upside budget to matter.",
                tickers=unique([*broad_growth, "HYG", "LQD"]),
                lookback_days=42,
                skip_days=0,
                top_n=5,
                min_return=0.000,
        ...
        127: (
            high_cagr_dual(
                name="i127_high_beta_etf_rotation",
                family="high_beta_broad_reentry",
                hypothesis="Diversified high-beta ETFs may capture recovery convexity without owning a narrow AI stock basket.",
                tickers=high_beta_etfs,
                top_n=4,
                min_return=0.005,
                max_asset_weight=0.22,
                vol_target=0.24,
            ...
            high_cagr_dual(
                name="i127_high_beta_drawdown_control",
                family="high_beta_broad_reentry",
                hypothesis="Pair diversified high beta with classic drawdown control instead of permanent scenario drag.",
                tickers=high_beta_etfs,
                top_n=4,
                min_return=0.005,
                max_asset_weight=0.22,
            ...
            _dip_overlay_candidate(
                name="i127_high_beta_washout_reentry",
                family="high_beta_broad_reentry",
                hypothesis="Only buy diversified high beta after a real washout and initial recovery evidence.",
                tickers=unique([*high_beta_etfs, "HYG", "LQD"]),
                lookback_days=42,
                skip_days=0,
                top_n=4,
                min_return=0.005,
            ...
            high_cagr_dual(
                name="i127_high_beta_low_churn",
                family="high_beta_broad_reentry_operable",
                hypothesis="Smooth diversified high-beta signals so upside capture is not purchased with constant small trades.",
                tickers=high_beta_etfs,
                top_n=4,
                min_return=0.005,
                max_asset_weight=0.22,
        ...
        128: (
            high_cagr_dual(
                name="i128_quality_compounder_rotation",
                family="quality_compounder_reentry",
                hypothesis="High-quality compounder stocks can supply upside without relying solely on semis or AI infrastructure.",
            ...
            high_cagr_dual(
                name="i128_non_ai_compounder_rotation",
                family="quality_compounder_no_ai",
                hypothesis="Remove obvious AI mega-cap dependence and test financials, health care, consumer, value, and quality exposure.",
                tickers=non_ai_compounders,
                top_n=5,
                min_return=0.005,
                max_asset_weight=0.20,
                vol_target=0.18,
            ...
            _dip_overlay_candidate(
                name="i128_compounder_dip_reentry",
                family="quality_compounder_reentry",
                hypothesis="Metered re-entry into quality compounders tests whether stocks beat ETFs in recoveries without too much concentration.",
                tickers=unique([*quality_compounders, "SPY", "RSP", "HYG", "LQD"]),
                lookback_days=42,
                skip_days=0,
                top_n=6,
                min_return=0.005,
            ...
            high_cagr_dual(
                name="i128_compounder_sanity_sized",
                family="quality_compounder_reentry",
                hypothesis="Keep quality compounder upside but cap news/scenario-only defensive moves unless market confirmation breaks.",
                tickers=quality_compounders,
                top_n=5,
                min_return=0.005,
                max_asset_weight=0.18,
        ...
        129: (
            high_cagr_dual(
                name="i129_sector_theme_recovery",
                family="sector_theme_reentry",
                hypothesis="Defense, biotech, uranium, energy, housing, retail, and materials may lead recoveries that are not AI-led.",
                tickers=sector_themes,
                top_n=5,
                min_return=0.005,
                max_asset_weight=0.22,
                vol_target=0.22,
            ...
            _sector_regime_candidate(
                name="i129_sector_theme_regime_reentry",
                family="sector_theme_reentry",
                hypothesis="Let the regime engine pick among non-AI themes, cyclicals, and defensive assets during recovery phases.",
                tickers=sector_themes,
                lookback_days=42,
                skip_days=0,
                top_n=5,
            ...
            _dip_overlay_candidate(
                name="i129_sector_theme_dip_reentry",
                family="sector_theme_reentry",
                hypothesis="Buy sector-theme washouts in steps when recovery evidence appears, without forcing QQQ exposure.",
                tickers=unique([*sector_themes, "HYG", "LQD"]),
                lookback_days=42,
                skip_days=0,
                top_n=5,
                min_return=0.005,
            ...
            high_cagr_dual(
                name="i129_sector_theme_low_churn",
                family="sector_theme_reentry_operable",
                hypothesis="Smooth sector-theme recovery so it can be monitored as a real paper candidate.",
                tickers=sector_themes,
                top_n=5,
                min_return=0.005,
                max_asset_weight=0.22,
        ...
        130: (
            _ai_cycle_candidate(
                name="i130_ai_capped_broad_core_25",
                family="ai_capped_broad_reentry",
                hypothesis="Keep an AI sleeve for upside, but cap it at 25% so the recovery engine cannot become all AI.",
                core_tickers=broad_core,
                satellite_tickers=ai_satellite,
                lookback_days=42,
                skip_days=0,
                top_n=5,
                min_return=0.005,
            ...
            _ai_cycle_candidate(
                name="i130_ai_capped_broad_core_40",
                family="ai_capped_broad_reentry",
                hypothesis="Allow more AI upside than the 25% cap, but still force broad/factor/cyclical exposure to dominate.",
                core_tickers=broad_core,
                satellite_tickers=ai_satellite,
                lookback_days=42,
                skip_days=0,
            ...
            _ai_cycle_candidate(
                name="i130_ai_capped_sanity_sized_40",
                family="ai_capped_broad_reentry",
                hypothesis="Test a 40% AI cap with scenario sizing restrained by the decision-sanity overlay.",
                core_tickers=broad_core,
                satellite_tickers=ai_satellite,
                lookback_days=42,
                skip_days=0,
            ...
            high_cagr_dual(
                name="i130_broad_core_plus_ai_etfs_only",
                family="ai_capped_broad_reentry",
                hypothesis="Use QQQ/SMH/SOXX only as ETFs inside a broad core, avoiding single-name AI concentration.",
                tickers=unique([*broad_core, "QQQ", "SMH", "SOXX", "XLK", "XLC", "XLY"]),
                top_n=7,
                min_return=0.005,
                max_asset_weight=0.18,
                vol_target=0.22,
        ...
        131: (
            high_cagr_dual(
                name="i131_fast_broad_growth_21d",
                family="fast_broad_growth_reentry",
                hypothesis="A 21-day broad-growth pulse checks whether recovery capture needs much faster signal windows.",
                tickers=broad_growth,
                lookback_days=21,
                top_n=6,
                min_return=0.000,
            ...
            high_cagr_dual(
                name="i131_fast_high_beta_21d",
                family="fast_high_beta_reentry",
                hypothesis="Fast high-beta pulse tests whether bottoms require earlier risk acceptance than 42-63 day signals.",
                tickers=high_beta_etfs,
                lookback_days=21,
                top_n=5,
                min_return=0.000,
            ...
            _dip_overlay_candidate(
                name="i131_fast_broad_growth_dip",
                family="fast_broad_growth_reentry",
                hypothesis="Combine a fast broad-growth signal with a dip overlay to enter before full trend confirmation.",
                tickers=unique([*broad_growth, "HYG", "LQD"]),
                lookback_days=21,
                skip_days=0,
                top_n=6,
            ...
            high_cagr_dual(
                name="i131_fast_compounder_21d",
                family="fast_quality_compounder_reentry",
                hypothesis="Fast compounder rotation tests whether high-quality stocks repair earlier than broad ETFs.",
                tickers=quality_compounders,
                lookback_days=21,
                top_n=5,
                min_return=0.000,
        ...
        132: (
            high_cagr_dual(
                name="i132_broad_growth_drawdown_only",
                family="drawdown_only_high_cagr",
                hypothesis="Replace scenario sizing with drawdown-only control to avoid staying too bearish during recoveries.",
                tickers=broad_growth,
                top_n=6,
                max_asset_weight=0.20,
                vol_target=0.22,
            ...
            high_cagr_dual(
                name="i132_high_beta_drawdown_only",
                family="drawdown_only_high_cagr",
                hypothesis="High-beta with drawdown-only control tests if upside can beat broad ETFs without permanent cash drag.",
                tickers=high_beta_etfs,
                top_n=4,
                min_return=0.005,
                max_asset_weight=0.20,
            ...
            high_cagr_dual(
                name="i132_compounder_drawdown_only",
                family="drawdown_only_high_cagr",
                hypothesis="Quality compounder momentum with drawdown control tests stock-selection upside with explicit off-ramp.",
                tickers=quality_compounders,
                top_n=5,
                min_return=0.005,
                max_asset_weight=0.18,
            ...
            high_cagr_dual(
                name="i132_sector_theme_drawdown_only",
                family="drawdown_only_high_cagr",
                hypothesis="Sector-theme momentum with drawdown control tests whether non-AI themes can provide enough upside.",
                tickers=sector_themes,
                top_n=5,
                min_return=0.005,
                max_asset_weight=0.20,
        ...
        133: (
            high_cagr_dual(
                name="i133_broad_growth_equal_top8",
                family="anti_concentration_high_cagr",
                hypothesis="Use many equal-weight winners to prevent a narrow AI stack while keeping broad growth exposure high.",
                tickers=unique([*broad_growth, *high_beta_etfs]),
                top_n=8,
                min_return=0.000,
                weighting="equal",
            ...
            high_cagr_dual(
                name="i133_compounder_equal_top8",
                family="anti_concentration_high_cagr",
                hypothesis="Equal-weight compounder rotation tests stock-level upside without one or two names dominating.",
                tickers=quality_compounders,
                top_n=8,
                min_return=0.000,
                weighting="equal",
            ...
            high_cagr_dual(
                name="i133_broad_growth_capped_sanity",
                family="anti_concentration_high_cagr",
                hypothesis="Combine concentration caps with decision-sanity so narrative risk cannot over-de-risk the sleeve.",
                tickers=unique([*broad_growth, *high_beta_etfs]),
                top_n=8,
                min_return=0.000,
                weighting="equal",
            ...
            high_cagr_dual(
                name="i133_non_ai_equal_top8",
                family="anti_concentration_no_ai",
                hypothesis="No-AI equal-weight recovery tests the strongest possible anti-bubble alternative before giving up on the idea.",
                tickers=unique([*non_ai_compounders, *sector_themes]),
                top_n=8,
                min_return=0.000,
                weighting="equal",
                max_asset_weight=0.14,
        ...
        134: (
            _dip_overlay_candidate(
                name="i134_aggressive_broad_growth_dip",
                family="aggressive_broad_reentry",
                hypothesis="Aggressive broad-growth dip re-entry tests whether the missing upside is simply under-sizing the rebound.",
                tickers=unique([*broad_growth, *high_beta_etfs, "HYG", "LQD"]),
                lookback_days=21,
                skip_days=0,
                top_n=7,
                min_return=0.000,
            ...
            _dip_overlay_candidate(
                name="i134_aggressive_compounder_dip",
                family="aggressive_compounder_reentry",
                hypothesis="Aggressive compounder dip re-entry checks whether stock-level leaders restore CAGR faster than ETFs.",
                tickers=unique([*quality_compounders, "SPY", "RSP", "HYG", "LQD"]),
                lookback_days=21,
                skip_days=0,
                top_n=7,
            ...
            _dip_overlay_candidate(
                name="i134_aggressive_non_ai_dip",
                family="aggressive_no_ai_reentry",
                hypothesis="Aggressive no-AI dip re-entry tests the best possible broadening recovery without AI exposure.",
                tickers=unique([*non_ai_compounders, *sector_themes, "HYG", "LQD"]),
                lookback_days=21,
                skip_days=0,
                top_n=7,
                min_return=0.000,
            ...
            high_cagr_dual(
                name="i134_aggressive_broad_growth_low_churn",
                family="aggressive_broad_reentry_operable",
                hypothesis="Aggressive broad-growth re-entry with hysteresis tests whether high CAGR can remain human-operable.",
                tickers=unique([*broad_growth, *high_beta_etfs]),
                lookback_days=21,
                top_n=7,
                min_return=0.000,
        ...
        135: (
            high_cagr_dual(
                name="i135_final_broad_growth_high_cagr",
                family="broadened_high_cagr_final",
                hypothesis="Final broad-growth candidate: high upside, capped positions, no scenario drag, and enough names to avoid narrow AI dependence.",
                tickers=unique([*broad_growth, *high_beta_etfs]),
                lookback_days=21,
                top_n=7,
                min_return=0.000,
            ...
            high_cagr_dual(
                name="i135_final_compounder_high_cagr",
                family="broadened_high_cagr_final",
                hypothesis="Final compounder candidate: stock-level quality/growth upside with concentration caps and fast re-entry.",
                tickers=quality_compounders,
                lookback_days=21,
                top_n=7,
                min_return=0.000,
            ...
            _ai_cycle_candidate(
                name="i135_final_ai_capped_broad_cycle",
                family="broadened_high_cagr_final",
                hypothesis="Final capped-AI cycle: preserve AI upside as one sleeve, but force broad risk-on assets to be the core.",
                core_tickers=unique([*broad_core, *broad_growth, *high_beta_etfs]),
                satellite_tickers=ai_satellite,
                lookback_days=21,
                skip_days=0,
                top_n=7,
...
def _multi_asset_risk_on_rotation_candidates(iteration: int) -> tuple[ExperimentCandidate, ...]:
    """Risk-on rotation across non-tech asset categories.

    This block tests whether the system can own risk when risk leadership is
    outside AI/technology. The design intentionally mixes growth, energy,
    commodities, industrials, defense, infrastructure, cyclicals, financials,
    defensive equity, global equities, credit, and precious metals. The goal is
    not to prove that every episode is repeatable; it is to test whether a
    category-aware risk-on universe can avoid collapsing into a permanent QQQ or
    AI trade while still capturing upside in inflation, energy, broadening, or
    global leadership regimes.
    """
    ...
    def multi_dual(
        *,
        ...
        min_return: float = 0.0,
        weighting: str = "risk_adjusted_score",
        max_asset_weight: float | None = 0.20,
        trend_filter_days: int | None = 63,
        ...
        return _candidate(
            name=name,
            role="multi_asset_risk_on_rotation",
            phase="multi_asset_risk_on_rotation",
            family=family,
            parent="multi_asset_rotation_research",
            hypothesis=hypothesis,
            scenario_sizing=scenario_sizing,
            decision_sanity=decision_sanity,
            strategy=StrategyConfig(
    ...
    batches: dict[int, tuple[ExperimentCandidate, ...]] = {
        136: (
            multi_dual(
                name="i136_multi_asset_fast_rotation",
                family="multi_asset_rotation_core",
                hypothesis="Fast relative strength across broad risk buckets should capture energy, cyclicals, or global leadership when tech lags.",
                tickers=all_risk_buckets,
                lookback_days=42,
                top_n=8,
                max_asset_weight=0.16,
                vol_target=0.20,
            ...
            multi_dual(
                name="i136_multi_asset_medium_rotation",
                family="multi_asset_rotation_core",
                hypothesis="Medium-speed rotation tests whether category leadership is more stable than a 21-42 day pulse.",
                tickers=all_risk_buckets,
                lookback_days=84,
                skip_days=5,
                top_n=8,
            ...
            multi_dual(
                name="i136_multi_asset_equal_top10",
                family="multi_asset_rotation_core",
                hypothesis="Equal-weight many-winner rotation avoids one category dominating the risk-on sleeve.",
                tickers=all_risk_buckets,
                lookback_days=42,
                top_n=10,
                weighting="equal",
            ...
            _sector_regime_candidate(
                name="i136_multi_asset_regime_rotation",
                family="multi_asset_rotation_core",
                hypothesis="Regime rotation across multi-asset risk buckets tests category switching plus dip-aware re-entry.",
                tickers=unique(all_risk_buckets),
                lookback_days=42,
                skip_days=0,
                top_n=8,
                min_return=0.000,
        ...
        137: (
            multi_dual(
                name="i137_no_tech_risk_rotation_fast",
                family="anti_tech_rotation",
                hypothesis="Explicitly exclude technology and AI to see whether non-tech risk-on leadership is tradable after tech drawdowns.",
                tickers=no_tech_risk,
                lookback_days=42,
                top_n=8,
                max_asset_weight=0.16,
                vol_target=0.20,
            ...
            multi_dual(
                name="i137_no_tech_drawdown_control",
                family="anti_tech_rotation",
                hypothesis="No-tech risk-on rotation with drawdown-only control tests whether non-tech leadership can be a true tech-crash alternative.",
                tickers=no_tech_risk,
                lookback_days=42,
                top_n=8,
                max_asset_weight=0.16,
            ...
            _dip_overlay_candidate(
                name="i137_no_tech_dip_reentry",
                family="anti_tech_rotation",
                hypothesis="If tech is the falling knife, buy non-tech risk-on baskets after broad drawdown repair instead.",
                tickers=unique([*no_tech_risk, "HYG", "LQD"]),
                lookback_days=42,
                skip_days=0,
                top_n=8,
                min_return=0.000,
            ...
            multi_dual(
                name="i137_no_tech_low_churn",
                family="anti_tech_rotation_operable",
                hypothesis="Low-churn no-tech risk-on rotation tests whether this can be monitored without daily portfolio noise.",
                tickers=no_tech_risk,
                lookback_days=63,
                top_n=8,
                max_asset_weight=0.16,
        ...
        138: (
            multi_dual(
                name="i138_energy_commodity_leadership",
                family="inflation_energy_rotation",
                hypothesis="Energy, commodities, metals, uranium, gold, and inflation hedges should dominate when inflation/energy shocks lead risk assets.",
                tickers=unique([*energy_commodities, "TIP", "UUP", "XLI", "XLU", "CCJ", "NRG"]),
                lookback_days=42,
                top_n=6,
                min_return=0.005,
                max_asset_weight=0.18,
            ...
            multi_dual(
                name="i138_energy_commodity_medium",
                family="inflation_energy_rotation",
                hypothesis="Medium-speed inflation/energy rotation tests whether these leadership cycles persist long enough for swing trades.",
                tickers=unique([*energy_commodities, "TIP", "UUP", "XLI", "XLU", "CCJ", "NRG"]),
                lookback_days=84,
                skip_days=5,
                top_n=6,
            ...
            _dip_overlay_candidate(
                name="i138_energy_commodity_dip_reentry",
                family="inflation_energy_rotation",
                hypothesis="Buy energy/commodity leadership after pullbacks only when repair appears, avoiding permanent commodity chasing.",
                tickers=unique([*energy_commodities, "TIP", "UUP", "HYG", "LQD"]),
                lookback_days=42,
                skip_days=0,
                top_n=6,
                min_return=0.005,
            ...
            multi_dual(
                name="i138_energy_commodity_drawdown_only",
                family="inflation_energy_rotation",
                hypothesis="Energy/commodity leadership with drawdown-only control tests whether the category can be a standalone risk-on sleeve.",
                tickers=unique([*energy_commodities, "TIP", "UUP", "XLI", "XLU", "CCJ", "NRG"]),
                lookback_days=42,
                top_n=6,
                min_return=0.005,
        ...
        139: (
            multi_dual(
                name="i139_cyclical_financial_broadening",
                family="cyclical_financial_rotation",
                hypothesis="Small/mid caps, banks, housing, retail, transports, and materials can lead when market breadth broadens away from tech.",
                tickers=cyclicals_financials,
                lookback_days=42,
                top_n=6,
                max_asset_weight=0.18,
                vol_target=0.20,
            ...
            _sector_regime_candidate(
                name="i139_cyclical_financial_regime",
                family="cyclical_financial_rotation",
                hypothesis="A regime-aware cyclical/bank basket tests whether economic repair phases can be captured outside QQQ.",
                tickers=cyclicals_financials,
                lookback_days=42,
                skip_days=0,
                top_n=6,
            ...
            multi_dual(
                name="i139_defensive_to_cyclical_bridge",
                family="cyclical_financial_rotation",
                hypothesis="Bridge from defensive equity into cyclicals when broadening improves, rather than jumping from cash to tech.",
                tickers=unique([*cyclicals_financials, *defensive_equity, "HYG", "LQD"]),
                lookback_days=42,
                top_n=8,
                max_asset_weight=0.16,
                vol_target=0.18,
            ...
            multi_dual(
                name="i139_cyclical_low_churn",
                family="cyclical_financial_rotation_operable",
                hypothesis="Cyclical/bank broadening with hysteresis tests whether the rotation survives human trading constraints.",
                tickers=cyclicals_financials,
                lookback_days=63,
                top_n=6,
                max_asset_weight=0.18,
        ...
        140: (
            multi_dual(
                name="i140_industrial_defense_infra_rotation",
                family="industrial_defense_infra_rotation",
                hypothesis="Defense, infrastructure, power, utilities, uranium, and industrials can be risk-on without being classic tech beta.",
                tickers=industrial_defense_infra,
                lookback_days=42,
                top_n=6,
                min_return=0.005,
                max_asset_weight=0.18,
            ...
            multi_dual(
                name="i140_power_infra_drawdown_only",
                family="industrial_defense_infra_rotation",
                hypothesis="Power/infrastructure with drawdown control tests whether AI-capex-adjacent but non-software assets are robust leaders.",
                tickers=industrial_defense_infra,
                lookback_days=42,
                top_n=6,
                min_return=0.005,
            ...
            _dip_overlay_candidate(
                name="i140_industrial_infra_dip_reentry",
                family="industrial_defense_infra_rotation",
                hypothesis="Buy industrial/defense/infrastructure dips after repair, a non-tech alternative to AI rebound chasing.",
                tickers=unique([*industrial_defense_infra, "HYG", "LQD"]),
                lookback_days=42,
                skip_days=0,
                top_n=6,
                min_return=0.005,
            ...
            multi_dual(
                name="i140_infra_quality_bridge",
                family="industrial_defense_infra_rotation",
                hypothesis="Blend infrastructure with quality/defensive equity to avoid chasing the most volatile energy or tech leaders.",
                tickers=unique([*industrial_defense_infra, *defensive_equity, "COWZ", "QUAL"]),
                lookback_days=42,
                top_n=8,
                max_asset_weight=0.16,
                vol_target=0.18,
        ...
        141: (
            multi_dual(
                name="i141_global_ex_us_risk_on",
                family="global_risk_on_rotation",
                hypothesis="Global and country ETFs test whether non-U.S. leadership can offset U.S. tech-specific drawdowns.",
                tickers=global_equity,
                lookback_days=42,
                top_n=6,
                max_asset_weight=0.18,
            ...
            multi_dual(
                name="i141_global_plus_commodity_bridge",
                family="global_risk_on_rotation",
                hypothesis="Blend global equities with commodities and dollar-sensitive assets for non-U.S./inflation leadership regimes.",
                tickers=unique([*global_equity, *energy_commodities, "UUP", "TIP"]),
                lookback_days=42,
                top_n=8,
                max_asset_weight=0.16,
                vol_target=0.20,
            ...
            _sector_regime_candidate(
                name="i141_global_regime_rotation",
                family="global_risk_on_rotation",
                hypothesis="Regime-aware global rotation tests whether international leadership is persistent enough after U.S. tech stress.",
                tickers=global_equity,
                lookback_days=42,
                skip_days=0,
                top_n=6,
            ...
            multi_dual(
                name="i141_global_low_churn",
                family="global_risk_on_rotation_operable",
                hypothesis="Low-churn global rotation checks whether global leadership is usable for human-triggered trades.",
                tickers=global_equity,
                lookback_days=63,
                top_n=6,
                max_asset_weight=0.18,
        ...
        142: (
            multi_dual(
                name="i142_credit_risk_on_rotation",
                family="credit_risk_on_rotation",
                hypothesis="Credit can behave like a risk-on repair sleeve when equities are fragile, so test HYG/JNK/loans/EMB plus duration.",
                tickers=credit_rates,
                lookback_days=42,
                top_n=5,
                max_asset_weight=0.22,
                vol_target=0.12,
            ...
            multi_dual(
                name="i142_credit_equity_bridge",
                family="credit_risk_on_rotation",
                hypothesis="Credit repair may lead equity repair; blend credit with broad equities and commodities as a risk-on bridge.",
                tickers=unique([*credit_rates, *broad_equity, *energy_commodities]),
                lookback_days=42,
                top_n=8,
                max_asset_weight=0.16,
                vol_target=0.16,
            ...
            _dip_overlay_candidate(
                name="i142_credit_led_dip_reentry",
                family="credit_risk_on_rotation",
                hypothesis="Use credit repair as the first re-risking sleeve after drawdowns before equities fully confirm.",
                tickers=unique([*credit_rates, *broad_equity, "GLD"]),
                lookback_days=42,
                skip_days=0,
                top_n=6,
                min_return=0.000,
            ...
            multi_dual(
                name="i142_credit_low_churn",
                family="credit_risk_on_rotation_operable",
                hypothesis="Low-churn credit rotation tests whether it is an operable bridge, not just a defensive drag.",
                tickers=credit_rates,
                lookback_days=63,
                top_n=5,
                max_asset_weight=0.22,
        ...
        144: (
            multi_dual(
                name="i144_macro_aware_multi_asset_sanity",
                family="multi_asset_sanity_sized",
                hypothesis="Multi-asset risk-on with scenario sizing restrained by decision sanity tests whether macro pressure can help without killing upside.",
                tickers=all_risk_buckets,
                lookback_days=42,
                top_n=10,
                max_asset_weight=0.12,
            ...
            multi_dual(
                name="i144_macro_aware_no_tech_sanity",
                family="multi_asset_sanity_sized",
                hypothesis="No-tech multi-asset rotation with restrained scenario sizing tests true anti-tech robustness.",
                tickers=no_tech_risk,
                lookback_days=42,
                top_n=8,
                max_asset_weight=0.16,
            ...
            multi_dual(
                name="i144_operable_multi_asset_low_churn",
                family="multi_asset_sanity_sized_operable",
                hypothesis="Low-churn multi-asset rotation tests whether category switching can be paper-monitored without excessive trade count.",
                tickers=all_risk_buckets,
                lookback_days=63,
                top_n=10,
                max_asset_weight=0.12,
            ...
            multi_dual(
                name="i144_operable_no_tech_low_churn",
                family="multi_asset_sanity_sized_operable",
                hypothesis="Low-churn no-tech rotation is the operability benchmark for anti-AI/anti-tech risk-on behavior.",
                tickers=no_tech_risk,
                lookback_days=63,
                top_n=8,
                max_asset_weight=0.16,
        ...
        145: (
            multi_dual(
                name="i145_final_multi_asset_fast",
                family="multi_asset_rotation_final",
                hypothesis="Final fast multi-asset candidate: category-aware risk-on breadth, capped positions, and enough speed to catch leadership shifts.",
                tickers=all_risk_buckets,
                lookback_days=42,
                top_n=10,
                max_asset_weight=0.12,
            ...
            multi_dual(
                name="i145_final_multi_asset_drawdown_only",
                family="multi_asset_rotation_final",
                hypothesis="Final multi-asset candidate with drawdown-only control, preserving risk-on optionality without narrative de-risking.",
                tickers=all_risk_buckets,
                lookback_days=42,
                top_n=10,
                max_asset_weight=0.12,
            ...
            multi_dual(
                name="i145_final_no_tech_multi_asset",
                family="multi_asset_rotation_final_no_tech",
                hypothesis="Final no-tech candidate: the strongest current test for risk-on leadership that avoids AI/tech as the core engine.",
                tickers=no_tech_risk,
                lookback_days=42,
                top_n=8,
                max_asset_weight=0.16,
            ...
            _sector_regime_candidate(
                name="i145_final_multi_asset_regime",
                family="multi_asset_rotation_final",
                hypothesis="Final regime candidate: multi-asset risk-on selection plus dip-aware re-entry and category diversification.",
                tickers=all_risk_buckets,
                lookback_days=42,
                skip_days=0,
                top_n=10,
...
def _candidate(
    *,
    ...
    return ExperimentCandidate(
        ...
        parent=parent,
        hypothesis=hypothesis,
        strategy=apply_excluded_ticker_policy_to_strategy(strategy),
        scenario_sizing=scenario_sizing,
        future_state_model=future_state_model,
        strategy_drawdown_model=strategy_drawdown_model,
        decision_sanity=decision_sanity,
